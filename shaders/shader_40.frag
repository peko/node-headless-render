...
void clouds_map(
	...
	float dens = fbm_clouds(
		cloud.pos * 5.2343 + vec3(.35, 13.35, 2.67),
		2.0276, .55, .55);

	#define cld_coverage .29475675 // higher=less clouds
	#define cld_fuzzy .000335 // higher=fuzzy, lower=blockier
	dens *= smoothstep(cld_coverage, cld_coverage + cld_fuzzy, dens);

	dens *= band(.2, .25, .75, cloud.height);
	
	dens = pow(dens, 4.0) * 8.0;
...
void clouds_march(
	...
	for (int i = 0; i < steps; i++) {
		...
		cloud.height = (length(cloud.pos) - planet.radius) / max_height * 1.3;
		t += t_step;
...
vec3 render(
	...
	mat3 rot = mul(rotate_around_x(u_time * 1.), rot_y);
	mat3 rot_cloud_y = rotate_around_y(57.);
	mat3 rot_cloud = mul(rotate_around_x(u_time * 2.1), rot_cloud_y);
