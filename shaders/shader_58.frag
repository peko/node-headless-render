...
void main(void)
{
    ...
    vec3 sky = vec3(0.1, 0.15, 0.2);
    sky *= 0.9 * pow(fbmHQ(itsc.p), 2.0);
    lacc = max(lacc * 0.3 + 0.3, 0.0);
    vec3 cloud = pow(vec3(lacc), vec3(0.5, 0.5, 0.5) * 1.0);
