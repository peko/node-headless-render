...
void main( void ) 
{
    ...
    float an = 1.1 + 0.05*(iGlobalTime-10.0) - 7.0*mouse.x;
    float d= 4.5;
    ...
    if( t>0.0 )
    {
        ...
        vec3 q = mapP( pos );
        float occ = calcAO( pos, nor, gl_FragCoord.xy ); 
        occ = occ*occ;
...
//##    float ks = texCube( iChannel0, pos*1.5, nor, 4.0 ).x;
        ...
        // dust
        vec3 tcol = vec3 (0.7); //0.8+0.4*texCube( iChannel0, pos*8.0, nor, 4.0 ).xyz;
